...
def login():
    if request.method == "POST":
        ...
        if user is not None:
            ...
            alert("Invalid Patient ID or Password. Please Try again.")
...
def predict():
    ...
    if request.method == "POST":
        # get the user's data from the predict form
        tdate = date.today()
    ...
    if prediction == "Chronic":
        ...
        with open("static/images/chronic.jpg", "rb") as image_file:
            prediction_img = base64.b64encode(image_file.read()).decode("utf-8")
